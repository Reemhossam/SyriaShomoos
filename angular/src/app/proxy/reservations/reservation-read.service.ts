...
export class ReservationReadService {
  ...
  getDetailedStatistics = (input: ReservationFilterDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ReservationGridDto>>({
      method: 'GET',
      url: '/api/app/reservation-read/detailed-statistics',
      params: { guestName: input.guestName, property: input.property, identityType: input.identityType, identityNumber: input.identityNumber, unitNumber: input.unitNumber, fromDate: input.fromDate, toDate: input.toDate, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
