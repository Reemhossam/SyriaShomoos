...
namespace SyriaShomoos.Reservations
{
  public class ReservationAppService : ApplicationService
  {
    ...
    public ReservationAppService(IRepository<Reservation, Guid> repository, IRepository<BranchSource, Guid> branchRepository)
    {
    ...
    {
      var reservation = await _repository.FirstOrDefaultAsync(
        x => x.ExternalIdentifier == input.ExternalIdentifier);
      ...
      var branch = await _branchRepository.FirstOrDefaultAsync(
          x => x.BranchCode == input.BranchCode);
